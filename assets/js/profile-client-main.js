/**
 * [FILE] profile-client-main.js
 * [POS] 客户用户中心入口 - 初始化所有组件和事件
 * [IN] 无
 * [OUT] 初始化后的页面
 ...
 *
 * Client Profile Page Main Entry
 * ============================================
 */
...
import { getRoleConfig, formatValue } from './profile-config.js';
...
const VALID_SECTIONS = ['overview', 'my-tasks', 'messages'];
...
async function renderRecentTasks() {
    ...
    try {
        ...
        if (tasks.length === 0) {
            container.innerHTML = '<p class="text-center text-gray-500">暂无任务</p>';
            return;
        }
        ...
        container.innerHTML = `
            ...
            <a href="#" class="btn btn-secondary btn-block" data-section="my-tasks">查看全部</a>
        `;
    } catch (error) {
...
function renderTaskSummary(task) {
    const extension = task.clientExtension || {};
    return `
        <div class="profile-task-card">
            ...
            <div class="profile-task-meta">
                ...
                <span>${extension.applicationsCount || 0} 人竞标</span>
            </div>
        </div>
    `;
}
